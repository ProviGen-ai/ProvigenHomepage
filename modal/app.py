...
class TxGemmaModel:
    ...
    def setup(self):
        ...
        # Cache tokenizer once at startup
    ...
    async def chat(self, messages: list[dict], max_tokens: int = 6144, temperature: float = 0.3) -> dict:
        """Run chat completion using AsyncLLMEngine with continuous batching."""
        import asyncio
        import time
        import os
        import uuid
        ...
        try:
            ...
            # --- Tokenize and prepare prompt (CPU-bound, fast) ---
            MAX_CTX = 8192
            RESERVED_OUTPUT = 512
            ...
            # --- Async generate with continuous batching ---
            ...
            # After first response, swap to compiled engine in background
            if self._needs_swap:
                self._needs_swap = False
                import threading
                def _background_swap():
                    try:
                        from vllm import AsyncEngineArgs as _AEA, AsyncLLMEngine as _ALE
                        print("[TxGemma] Background: building compiled 8-bit engine...")
                        model_dir = os.path.join(TXGEMMA_WEIGHTS_PATH, "txgemma-27b-chat")
                        compiled_args = _AEA(
                            model=model_dir,
                            max_model_len=8192,
                        ...
                        compiled_engine = _ALE.from_engine_args(compiled_args)
                        self._swapping = True
                        import time as _t
                        _t.sleep(1)
                        old = self.engine
                        ...
                        self._swapping = False
                        print("[TxGemma] Swap complete — now serving compiled 8-bit engine, cache saved")
                    except Exception as ex:
                        self._swapping = False
