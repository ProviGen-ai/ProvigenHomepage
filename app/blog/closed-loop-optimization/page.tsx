...
export default function ClosedLoopOptimizationPost() {
  return (
      ...
      <article className="pb-24 pt-[160px]">
        <div className="max-w-3xl mx-auto px-4 md:px-12">
          ...
          <div className="prose-blog">
            <p>
              Most experimental optimization still works like this: run a batch,
              look at the results, discuss what to try next, repeat. The
              learning happens in the scientist&apos;s head. The question is
              what changes when you make that loop explicit and let every round
              inform the next one automatically.
            </p>

            <p>
              Experimental optimization is often described as if it were a
              sequence of isolated screens. In reality, most teams are running a{" "}
              <em>campaign</em>. They start with an objective, test a first
              batch of conditions, learn from the results, and adjust the next
              round accordingly. The difference is that in most labs today, this
              learning loop is still largely manual. Closed-loop optimization
              makes it explicit: each round updates the system&apos;s
              understanding of what looks promising, what remains uncertain, and
              where the next experiments are likely to be most valuable.
            </p>

            <p>
              The exact shape of that loop depends on the application. In assay
              optimization, the objective may be plate-level robustness rather
              than single-well performance. In protein engineering, early in
              vitro data may arrive quickly while in vivo outcomes lag behind by
              weeks. In media optimization, the challenge is often not a single
              dominant factor but a dense network of interactions. Across all
              three, the principle is the same: use each experimental round to
              make the next one more informed.
            </p>
            ...
            <p>
              At the most simplified level, every campaign works like this: run
              a batch of experiments, update a probabilistic model of the design
              space, and choose the next batch based on both expected
              performance and remaining uncertainty. The model does not just ask{" "}
              <em>where do I expect the best result?</em> It also asks{" "}
              <em>where am I still most uncertain?</em> Balancing these two
              questions is what distinguishes an active learning campaign from a
              static screen.
            </p>
          </div>
          ...
          <figure className="my-12">
            ...
            <figcaption className="mt-3 font-mono text-xs text-[#6c7793] leading-relaxed">
              A one-dimensional illustration of Bayesian optimization. The blue
              curve is the model&apos;s belief about the objective; the shaded
              region represents uncertainty. After each observation the model
              updates and selects the next query by balancing expected
          ...
          <div className="prose-blog">
            <p>
              ...
              operate in much higher-dimensional spaces with noisy measurements,
              batch constraints, and practical lab realities. But the basic
              decision logic is the same, and it is this logic that drives the
              three campaign stories below.
            </p>

            {/* ---- Story 1: Assay ---- */}
            <h2>Turning assay troubleshooting into a learning campaign</h2>

            <p>
              Imagine a team preparing to scale an assay into routine screening.
              The assay works, but not robustly enough. CV is higher than
              acceptable, edge effects appear on some plates, and the signal
              window narrows unpredictably between runs. The usual response is
              iterative troubleshooting: adjust one variable, rerun, check the
              result, adjust again. It works, but it is slow, and the path
              through parameter space is driven by intuition rather than by
              systematic evidence.
            </p>
            ...
            <p>
              ...
              replicates on a plate. That matters because the objective is not a
              raw signal value from one well. It is reproducibility,
              variability, and spatial stability across the plate.
            </p>

            <p>
              Once the first plates are run, the system ingests the resulting
              data. That includes not only assay readouts, but potentially also
              execution metadata: dispense timing, shaker behavior, temperature
              traces, liquid handling logs, or other instrument-level
              information that may explain performance variation. The raw well
              data is then aggregated into condition-level outcomes such as CV,
              Z&prime;, signal-to-background, drift across plate positions, or
              failure modes.
            </p>
            ...
            <p>
              ...
              are close variations around the best performers so far. Others are
              selected because the model is still uncertain there and believes
              more information would be valuable.
            </p>

            <p>
              As the campaign progresses, the behavior changes. Early rounds are
              broader and meant to map the space. Later rounds become more
              targeted, focusing on a narrower incubation window or a tighter
              reagent concentration range where the model sees the best tradeoff
              between robustness and signal quality. Eventually, the campaign
              converges on a protocol region that is both strong and stable. The
              output is not just &ldquo;this one setting worked once,&rdquo; but
              a better-characterized operating window with evidence that it is
              robust across repeated execution.
            </p>

            <aside>
              In this type of campaign, a single optimization sample is
              typically one assay condition summarized from a plate or replicate
              layout, not one well. The wells are raw observations feeding into
              that condition-level outcome.
            </aside>
            ...
            <p>
              ...
              stability, aggregation, manufacturability, and eventually in vivo
            ...
            <p>
              ...
              point, the system has already learned something useful even before
              any in vivo work has started. It may recognize that a particular
              region of sequence space consistently gives poor
            ...
            <p>
              ...
              the campaign becomes explicitly multi-stage and time-delayed. The
              important point is that the campaign does not stop and wait
            ...
            <p>
              ...
              accumulated in vitro and developability data. Now the model can do
              something much more valuable than simply marking winners and
            ...
            <p>
              That changes later rounds materially. Before in vivo feedback, the
              model is guided mainly by in vitro and developability signals.
              After enough in vivo data has been observed, it becomes much
              better calibrated toward the outcomes that matter most clinically.
              Candidate selection becomes more informed because the model is no
              longer just optimizing for strong proxy performance. It is
            ...
            <h2>
              Navigating complex media interactions without exhaustive screening
            </h2>
            ...
            <p>
              ...
              The objective might be higher viable cell density, better phenotype
              retention, improved differentiation efficiency, stronger
              productivity, or some weighted combination of these.
            </p>

            <p>
              The first round samples a broad range of media and process
              conditions. Each candidate condition is run with replicates, often
              across a plate-based or small-scale format. At this stage, the
              goal is not only to find a few good conditions but to map how
              sensitive the cells are to different regions of the formulation and
              process space.
            </p>

            <p>
              After the first batch is executed, the system collects the
              resulting measurements: cell counts, viability, morphology-derived
              features, marker expression, metabolic readouts, productivity
              signals, or other assay outputs. As with the other campaign types,
              the system can also incorporate process traces and device logs from
              the relevant instruments where they help explain
              execution-dependent variation.
            </p>
            ...
            <p>
              ...
              objective. Others are chosen because they help resolve uncertainty
              about important interactions, for example whether a certain growth
              factor is only beneficial above a certain basal nutrient level, or
              whether a feeding schedule changes the effect of another
              supplement.
            </p>
            ...
            {/* ---- Summary ---- */}
            <h2>The same loop, different decision boundaries</h2>
          </div>
              ...
              <tbody className="text-[#090E34]">
                <tr className="border-b border-[#e8e6e1]">
                  ...
                  <td className="py-3 pr-4">
                    Condition at plate level
                  </td>
                  <td className="py-3">
                    Well readouts, replicates, spatial effects, instrument logs
                  </td>
                </tr>
                <tr className="border-b border-[#e8e6e1]">
          ...
          <div className="prose-blog">
            <p>
              Across all three applications, the campaign structure is the same.
              What changes is the level at which a candidate is defined, what
              data feeds into each evaluation, and how delayed or hierarchical
              outcomes affect the learning loop. The right sample definition
              follows the decision boundary, not the instrument boundary.
              Replicates improve confidence in a sample. They do not redefine
              what the sample is.
            </p>

            {/* ---- Data richness ---- */}
            <h2>Beyond single endpoints</h2>
            ...
            <p>
              ...
              outputs, execution metadata, device logs from related instruments,
              and, where relevant, delayed downstream results such as in vivo
              readouts. The richer the evidence per candidate, the faster the
              model can distinguish real performance differences from noise.
            </p>

            <p>
              From the user&apos;s perspective, however, the campaign is still
              experienced as a sequence of candidate decisions, observed results,
              and increasingly informed next-round recommendations. The
              complexity of the underlying data stack does not surface as
              complexity in the workflow. It surfaces as better decisions, faster
              convergence, and fewer wasted experiments.
            </p>
            ...
            <p>
              The point is not to &ldquo;run Bayesian optimization&rdquo; as an
              abstract algorithm. It is to make each experimental round more
              informed than the last while using all relevant evidence generated
              by the workflow. Whether the campaign is about tightening an
              assay, engineering a therapeutic protein, or finding a better
              formulation, the operating principle is the same: structured
              learning, round over round, converging on outcomes that matter.
            </p>

            <p>
              If you are interested in exploring what a closed-loop campaign
              might look like for your specific workflow, reach out at{" "}
              <a href="mailto:contact@provigen.ai">contact@provigen.ai</a>.
            </p>
          </div>
        </div>
      </article>
